...
@section('title', t('purchase_return.new_title'))

@section('content')
    <div class="row justify-content-center">
        <div class="col-lg-9 col-xl-8">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="fw-bold mb-0">{{ t('purchase_return.new_title') }}</h4>
            ...
            <div class="card mb-3">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-6">
                            <small class="text-muted">{{ t('purchase_return.original_invoice') }}</small>
                    ...
                    @if ($purchase->supplier)
                        <div>
                            <small class="text-muted">{{ t('nav.suppliers') }}</small>
                            <span class="fw-semibold ms-1">{{ $purchase->supplier->name }}</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('purchase-returns.store', $purchase) }}" id="returnForm">
                @csrf
                            ...
                            <table class="table table-sm mb-0 align-middle">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="text-center">{{ t('purchase_return.purchased_qty') }}</th>
                                        <th class="text-center">{{ t('purchase_return.returnable_qty') }}</th>
                                        <th class="text-center" style="width:120px">{{ t('purchase_return.return_qty') }}</th>
                                ...
                                <tfoot class="table-light">
                                    <tr>
                                        <th colspan="4" class="text-end">{{ t('purchase_return.return_total') }}</th>
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-12">
                                <label for="reason" class="form-label">{{ t('purchase_return.reason') }} <span class="text-muted">({{ t('common.optional') }})</span></label>
                                <input type="text" id="reason" name="reason" class="form-control"
                                    value="{{ old('reason') }}" placeholder="{{ t('purchase_return.reason_ph') }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <button type="button" class="btn btn-outline-secondary" id="returnAllBtn">
                        <i class="mdi mdi-select-all me-1"></i> {{ t('purchase_return.return_all') }}
                    </button>
                    <button type="submit" class="btn btn-danger" id="submitBtn" disabled>
                        <i class="mdi mdi-check me-1"></i> {{ t('purchase_return.confirm_btn') }}
                    </button>
                </div>
            </form>
        </div>

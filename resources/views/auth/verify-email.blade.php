...
<div class="card" style="max-width: 400px; width: 100%;">
    <div class="card-body p-4">
        ...
        <form method="POST" action="{{ route('verification.code') }}" class="mb-3">
            ...
            <button type="submit" class="btn btn-primary d-grid w-100">{{ t('authpage.verify_code_btn') }}</button>
        </form>
        ...
        <div class="row g-2">
            <div class="col-7">
                <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <button type="submit" class="btn btn-outline-primary d-grid w-100">{{ t('authpage.verify_resend_btn') }}</button>
                </form>
            </div>
            <div class="col-5">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="btn btn-outline-secondary d-grid w-100">{{ t('authpage.verify_logout') }}</button>
                </form>
            </div>
        </div>
    </div>
